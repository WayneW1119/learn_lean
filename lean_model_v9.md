...
> **相对于 v8 的变化：**
>
> * §5a：预置状态完全取消；Sigma 和 Prod 都通过 StructureCmd 定义（SΣ 和 SΣΠ 变为方便缩写）
> * §5b（新增章节）：类型生成辅助函数 `make_forall_chain`、`make_type_app`、`make_structure_type`、`make_constructor_type`
> * §6：新增 `StructureCmd` 命令、`ParamDecl` 和 `FieldDecl` 数据类型
> * §7：`exec` 新增 `StructureCmd` 分支 — 自动生成 type constructor、constructor 和 StructureInfo
> * §15：前置状态 `S` 现在包含 `StructureCmd Prod` 步骤
> * §15b（新增章节）：`StructureCmd` 的完整例子（Point、UnitLike、Prod）
> * §19.6 / §20.5：新增 Point 的 whnf 和 defeq 例子
> * v9 不改 `CoreTerm`（7 个构造子不变），不改 kernel 函数（whnf、infer_type、defeq、projection_type 不变）
>
...
- `CoreTerm` 有 7 个构造子：`Const`、`Sort`、`Var`、`App`、`Lam`、`Forall`、`Proj`（v9 不变）。
...
- v9 新增 `StructureCmd`（§6），用户可以通过顶层命令声明新结构（包括 Sigma 和 Prod）。v9 不再预置任何结构 — 所有 StructureInfo 都通过 StructureCmd 生成。Kernel 规则无需修改即可处理新声明的结构。
---
## 1. 关于 `PartialMap`
...
- 当前模型**不引入** `ProjectionInfo`。`StructureInfo` 记录 `typeName`、`constructor`、`params`、`fields`（见 §5）。`numParams = len(params)`、`numFields = len(fields)` 不再单独存储。
...
`Proj` 用一个构造子解决了所有结构的字段提取。这就是"一般化"的具体收益。v9 中所有结构（包括 Sigma）都通过 StructureCmd 定义，Proj 规则自动适用 — 不需要修改 kernel 代码（见 §15b 和 §19.6 的 Point 例子）。
...
**评论：** 当前模型不引入 `ProjectionInfo`，不记录字段名字的全局投影名（如 `Sigma.fst`）。`Proj(structName, index, target)` 已经编码了投影的全部信息。将来如果要更接近 Lean 的 projection declaration 机制，再把 `Sigma.fst / Sigma.snd` 作为全局名字加入。

### 5.2a `StructureInfo` well-formedness invariant           【v9 修改】
...
**评论：** v9 中所有 StructureInfo 都通过 `exec(StructureCmd)` 生成 — telescope 检查保证每个参数/字段类型都是 Sort（即 universe level 可计算），参数和字段的名字互不重复，字段类型模板中的 Var 引用只指向参数名或前序字段名。没有 StructureInfo 是预置的。
...
注意：`S.Structures.get(Sigma)` 不是天生存在的。它必须来自某个具体状态。v9 中 Sigma 的 StructureInfo 来自 `exec(StructureCmd Sigma ...)`（见 §15b.1），不再预置。
...
所有结构（包括 Sigma 和 Prod）都通过 StructureCmd 定义（见 §15b）。
...
这些名字在 StructureCmd 中由用户提供（见 §6）。并要求所有名字互不相同。

当前模型暂时**不引入** `Sigma.fst / Sigma.snd / Prod.fst / Prod.snd`。因为 `Proj(Sigma, 0, p)` 和 `Proj(Sigma, 1, p)` 已经表达了投影。
...
**评论：** `Sigma` 不是模型宇宙里自动存在的魔法对象。v9 中 `Sigma` 的类型签名和 StructureInfo 由 `exec(StructureCmd Sigma Sigma.mk ...)` 自动生成 — 不再预置。这类似于 Lean 的 prelude，但 prelude 中的结构声明在 v9 中由 StructureCmd 显式执行，而非隐式硬编码。
...
def make_structure_type(info : StructureInfo) -> CoreTerm:
    ...
    v9 简化：所有结构的结果类型固定为 Sort(level 0)。
...
**评论（简化版 universe）：** 当前模型暂时只支持单宇宙结构 — 所有 StructureCmd 生成的结构类型结果都固定为 `Sort(level 0)`。这不是通过参数类型的 universe 自动推导出来的，而是 v9 的简化约定。例如 Sigma 中参数 B 的类型 `Π (x : A), Sort 0` 本身生活在 Sort 1，但 `Sigma A B` 仍被本模型规定为 `Sort 0`。完整实现需要显式处理 universe 参数和 result sort，或进行 universe constraint inference；结构结果 universe 不是简单从 `param.type` 的 universe 自动取 `max` 得到（否则 Sigma 会得到 Sort 1，而非期望的 Sort 0）。v9 暂时不处理这些机制，直接固定结构结果为 `Sort(level 0)`。Universe polymorphism 和更真实的 universe inference 以后再加。
...
**评论：** StructureCmd 是 v9 的核心新增。它让用户声明一个 structure，exec 会自动生成 type constructor 的类型签名、constructor 的类型签名，以及 StructureInfo。用户必须同时给出 typeName 和 constructor 名字（如 Point 和 Point.mk）— 这避免了自动命名策略的复杂性。
...
### 7.4 规则 3：执行 `StructureCmd`                            【v9 新增】
...
**第一步：名字检查** — `typeName ≠ constructor`（避免同名导致状态更新时覆盖），且 `typeName` 和 `constructor` 不与已有全局名字冲突。
...
S_tmp = State(Axioms = S.Axioms.set(typeName, type_type), Defs = S.Defs, Structures = S.Structures)
...
- StructureCmd 是 State 生成器，不产生 CoreTerm。它修改 State（加两个 Axiom、加一个 StructureInfo），之后所有已有的 kernel 规则自动生效。
...
def exec(S: State, cmd: Command) -> State:
    ...
    v9：exec 新增 StructureCmd 分支；
    AxiomCmd / DefCmd 不修改 Structures；
    StructureCmd 会新增 typeName、constructor（Axioms）和 StructureInfo（Structures）。
    '''
    ...
    if isinstance(cmd, AxiomCmd):
        ...
        if typeName in S.Axioms or typeName in S.Defs or typeName in S.Structures:
            raise Error
        if constructor in S.Axioms or constructor in S.Defs or constructor in S.Structures:
            raise Error
...
- **Sigma formation 的类型推断路径：**
  ...
  3. `infer_type(S, Γ, App(App(Const Sigma, A), Bfun))` → 继续 App 规则链，最终得到 Sigma 的结果类型。在当前单宇宙 v9 中，`Sigma A Bfun` 的类型由 `Const Sigma` 的签名推出，结果是 `Sort(level 0)`。更一般的 universe-polymorphic 版本以后再加。
...
为了后续展示 `elab` / `whnf` / `defeq` / `StructureCmd` 的例子，先构建一个共同的前置状态 `S`。

v9 不预置任何结构。从空初始状态 `S₀`（§5a）开始，先通过 StructureCmd 定义 Sigma 和 Prod，再添加 axiom。
...
**评论：** 后续 §18-§21 的所有例子都基于这个状态 `S`。需要时会在用到的地方临时扩展状态（如 §19.4 加入 `pair0`，§19.6 加入 `Point`）。v9 中 Sigma 和 Prod 都不再预置 — 全部通过 StructureCmd 定义，类型签名和 StructureInfo 由 exec 自动生成（完整推导见 §15b）。

---

## 15b. StructureCmd 例子                                    【v9 新增】

本节展示 `StructureCmd` 的完整执行过程，逐步骤对照 §7.4 中的四步规则。

### 15b.1 StructureCmd Sigma（依值结构）— v9 的核心例子
...
`make_structure_type` 按 v9 的单宇宙简化规则，直接把结构结果类型固定为 `Sort(level 0)`（不计算 result_level）：
...
**评论：** 这是 v9 的核心进展 — Sigma 不再是预置的魔法对象，而是通过 StructureCmd 定义出来的。它与 Prod、Point 使用完全相同的机制（四步执行：名字检查 → telescope 检查 → 类型生成 → 状态更新）。唯一的区别是 Sigma 有依值参数和依值字段（B 引用 A，b 引用 a 和 B），而 Prod 和 Point 没有 — 但 StructureCmd 的 telescope 检查逐步扩展上下文，天然支持依值类型。

elab 的 `SSigma`/`SPair`/`SFst`/`SSnd` 分支仍然硬编码引用 `Const Sigma` — 这是 surface 层的 Sigma 专属语法，不是 StructureCmd 的限制。其他结构的 surface 语法是后续版本的工作。
...
**对比：** 这与 v8 中预置的 Prod 和 Prod.mk 类型完全相同。StructureInfo 的 params/fields 也完全相同。这验证了 StructureCmd 可以替代预置方式 — 从 v9 起，Prod 不再预置，而是通过 StructureCmd 定义。
...
- **在 v9 中，`SSigma` 不是无条件可用的 primitive。** 它的 elaboration 结果引用 `Const Sigma`；因此必须先执行 StructureCmd Sigma（见 §15b.1），使 `Sigma` 和 `Sigma.mk` 出现在 State 中，后续类型检查才能成功。如果用户在 StructureCmd Sigma 执行之前就写 `SSigma(...)`，elab 会生成 `(Const Sigma) A Bfun`，但此时 `Sigma ∉ S.Axioms`，后续 `infer_type` 会失败。
...
**注意：** `elab` 仍只处理 Sigma 相关的 surface 构造子（`SSigma`、`SPair`、`SFst`、`SSnd`）。这些构造子硬编码引用 `Const Sigma` — 只在 StructureCmd Sigma 执行后才可用。其他结构的 surface 语法是后续版本的工作。
...
`StructureInfo` 记录 `params` 和 `fields` 的名字与类型模板。它使得 `whnf` 和 `infer_type` 的 `Proj` 分支都可以查找结构信息，不依赖硬编码。v9 中所有 StructureInfo 都通过 StructureCmd 生成，不再预置。
...
exec(StructureCmd) 调用辅助函数 + elab + infer_type（§7.4，v9 新增）：
...
**Prod 对应：** Prod.mk 应用 ↔ Lean 的 `Prod.mk` 应用；`Proj(Prod, 0, ...)` ↔ Lean 的 `Prod.fst`；`Proj(Prod, 1, ...)` ↔ Lean 的 `Prod.snd`。

### 17.5 架构变化

v9：Surface → Core（自 v8 沿用，但新增 StructureCmd — 所有结构（包括 Sigma）都通过 StructureCmd 定义，不再预置）。Kernel 规则（whnf、infer_type、defeq、projection_type）自 v8 不变。
...
沿用之前的状态 `S`（其中 `Prod` 和 `Prod.mk` 在 `Axioms` 中，`Prod` 在 `Structures` 中 — 前者通过 StructureCmd 定义，见 §15b.2）。
...
### 19.6 Point 的 whnf 例子                                    【v9 新增】

**前置状态**

在状态 `S` 的基础上，执行 `StructureCmd Point Point.mk params=[X : SSort 0] fields=[x : SVar X, y : SVar X]`（完整推导见 §15b.3），得到状态 `S'`：
...
S'.Defs = S.Defs
S'.Structures = S.Structures ∪ {
  Point : StructureInfo(Point, Point.mk,
    params=[ParamInfo(X, Sort(level 0))],
    fields=[FieldInfo(x, Var X), FieldInfo(y, Var X)])
}
```
...
```text
pt_nat = (Const Point.mk) (Const Nat) (Const zero) (Const zero)
```

即 `Point.mk Nat zero zero : Point Nat`。类型正确：`X = Nat`，`x = zero : Nat`，`y = zero : Nat`。

**例子 1：** `whnf(S', Proj(Point, 0, pt_nat))`

**推导过程：**

1. `t = Proj(Point, 0, pt_nat)`，匹配 `Proj` 分支 (§12)。

2. 先化简 target：

   ```text
   whnf(S', pt_nat) = pt_nat
   ```

   `pt_nat` 是以 `Const Point.mk` 为头部的嵌套 `App`。`Point.mk` 是 axiom（不在 `S'.Defs` 中），不能 delta 展开。函数部分是 `Const Point.mk`，不是 `Lam`，不触发 beta reduction。

3. 查结构信息：

   ```text
   S'.Structures.get(Point)
   =
   StructureInfo(
     typeName    = Point,
...
4. 拆开应用：
   ...
   ```text
   collect_app(pt_nat)
   = Pair(Const Point.mk, [Const Nat, Const zero, Const zero])
   ```

5. `head = Const Point.mk`，与 `constructor = Point.mk` 一致 ✓

6. 当前投影 index 是 `0`，所以字段位置是：

   ```text
   field_pos = len(info.params) + 0 = 1 + 0 = 1
   ```

7. 参数列表第 `1` 个参数是 `Const zero`。

8. 返回 `whnf(S', Const zero) = Const zero`。
...
**例子 2：** `whnf(S', Proj(Point, 1, pt_nat))`
...
7. 当前投影 index 是 `1`，所以字段位置是：
   ...
   ```text
   field_pos = len(info.params) + 1 = 1 + 1 = 2
   ```

8. 参数列表第 `2` 个参数是 `Const zero`。

9. 返回 `whnf(S', Const zero) = Const zero`。
...
**评论：** Point 有 1 个参数和 2 个字段，所以 `field_pos = 1 + index`。而 Sigma 有 2 个参数和 2 个字段，`field_pos = 2 + index`。Prod 有 2 个参数和 2 个字段，`field_pos = 2 + index`。`whnf` 机制完全由 `StructureInfo` 的参数和字段数量驱动，不需要任何代码修改。
...
### 20.5 Point 的 infer_type + defeq                                【v9 新增】

**前置状态**

沿用 §19.6 的状态 `S'`（包含 Point 结构信息）。令 `pt_nat` 表示 §19.6 中定义的 `Point.mk Nat zero zero`。

**例子 1：** `infer_type(S', ∅, Proj(Point, 0, pt_nat))`

1. `t = Proj(Point, 0, pt_nat)`，匹配 `Proj` 分支 (§14)。

2. 查结构信息：

   ```text
   info = S'.Structures.get(Point)
   = StructureInfo(Point, Point.mk,
     params=[ParamInfo(X, Sort(level 0))],
     fields=[FieldInfo(x, Var X), FieldInfo(y, Var X)])
   ```
...
3. 推断 target 类型：
   ...
   ```text
   infer_type(S', ∅, pt_nat) = App(Const Point, Const Nat)
   ```

   （`pt_nat` 是 `Point.mk` 的三参数应用；`Point.mk` 的类型是 `Π (X : Sort 0), Π (x : X), Π (y : X), Point X`，三参数依次应用后返回 `Point Nat`。）

4. 匹配结构类型：

   ```text
   match_structure_type(S', Point, App(Const Point, Const Nat))
   ```

   `whnf` 后 `head = Const Point`，`args = [Const Nat]`。`len(args) = 1 = len(info.params)` ✓。返回 `[Const Nat]`。

5. 计算投影类型：

   ```text
   projection_type(S', ∅, info, 0, pt_nat, [Const Nat])
   ```
   ...
   `subst_many(Var X, [(X, Const Nat)]) = Const Nat`
...
**例子 2：** `infer_type(S', ∅, Proj(Point, 1, pt_nat))`
...
5. 计算投影类型：
   ...
   ```text
   projection_type(S', ∅, info, 1, pt_nat, [Const Nat])
   ```

   `fields[1].type = Var X`（两个字段的类型模板相同 — 都是 `Var X`）
...
推导：`infer_type` 返回 `Const Nat`，与 expected type `Const Nat` 相同，`defeq` 直接比较成功 ✓。
...
**评论：** Point 的两个字段类型都是 `Var X`，替换 `X → Const Nat` 后都是 `Const Nat`。这比 Sigma（第二个字段是 `App(Var B, Var a)`，依值）和 Prod（第二个字段是 `Var B`，非依值但可能不同于第一个字段）更简单 — 两个字段类型完全相同。这再次验证了 `infer_type` 的 Proj 规则完全由 StructureInfo 的 FieldInfo.type 模板驱动，不需要对任何特定结构硬编码。
...
这由 `Sigma` 的类型签名（通过 StructureCmd 定义，见 §15b.1）以及连续两次 `App` 规则推出 ✓
...
这相当于"绕过 elab，直接写 kernel 层的 term"。在实际的 Lean 中，用户写 `⟨a, b⟩` 或 `(a, b)` 这样的语法，elaborator 会根据 expected type 决定是 `Sigma.mk` 还是 `Prod.mk` 的应用。v9 中 Sigma 和 Prod 都通过 StructureCmd 定义，但 `elab` 仍然只有 Sigma 的 surface 语法 — 要支持其他结构的 surface 语法，需要新增 SurfaceTerm 构造子或通用的 anonymous constructor 机制。

**评论：** v9 的 `StructureCmd` 解决了"声明结构"的问题（包括 Sigma 和 Prod），但 `elab` 仍然只对 Sigma 有 surface 语法（`SSigma`/`SPair`/`SFst`/`SSnd`）。Kernel 层（`whnf`、`infer_type`、`defeq`）对任何在 `S.Structures` 中注册的结构都适用 — 不需要代码修改。但 surface 层的自定义结构语法是后续版本的工作。
...
数据类型：ParamDecl, FieldDecl, ParamInfo, FieldInfo, StructureInfo

Command 构造子：AxiomCmd, DefCmd, StructureCmd                    【v9 新增 StructureCmd】

State 字段：Axioms, Defs, Structures

函数：FV(CoreTerm), subst(CoreTerm), subst_many, exec, infer_type, check_type, whnf, defeq, elab, elab_raw, is_sigma_type, collect_app, match_structure_type, projection_type, make_forall_chain, make_type_app, make_structure_type, make_constructor_type              【v9 新增 4 个类型生成辅助函数】
```
...
StructureCmd（用户声明新结构，exec 自动生成 type constructor 类型、constructor 类型、StructureInfo）  【v9 新增】
```
...
**评论：** `Proj` 的 whnf 规则由 `StructureInfo` 中的 `constructor`、`len(params)`、`len(fields)` 驱动 — 不需要对任何特定结构硬编码投影化简。添加新结构时，只要通过 StructureCmd 注册到 `Structures` 中，`Proj` 的 whnf 规则自动适用。

**v9 的关键进展：** **StructureCmd 让用户可以声明新结构，exec 自动生成类型签名和 StructureInfo。v9 不预置任何结构 — Sigma 和 Prod 都通过 StructureCmd 定义。** Kernel 层（whnf、infer_type、defeq）无需任何修改即可处理新声明的结构。elab 的 Sigma 专属 surface 语法（`SSigma`/`SPair`/`SFst`/`SSnd`）仍然硬编码 — 这些语法只在 StructureCmd Sigma 执行后才可用。自定义结构的 surface 语法是后续版本的工作。
...
> v8 是"让 Proj 的类型推断一般化" — Sigma 和 Prod 由同一个 StructureInfo + Proj 机制处理；
> v9 是"让所有结构都由用户声明" — StructureCmd 自动生成 StructureInfo（包括 Sigma），kernel 规则自动适用。不再预置任何结构。
>
> Kernel 层（whnf、infer_type、defeq）不变；
> Command 层新增 StructureCmd；
> exec 层新增 StructureCmd 的处理逻辑和类型生成辅助函数；
> Surface 层（elab）仍只处理 Sigma — 自定义结构的 surface 语法是后续版本的工作。
